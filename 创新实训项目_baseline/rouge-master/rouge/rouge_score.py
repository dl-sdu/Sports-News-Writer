...
'''def _split_into_words(sentences):
    print(list(itertools.chain(*[_.split(" ") for _ in sentences])))
    return list(itertools.chain(*[_.split(" ") for _ in sentences]))
'''
def segment(sentence):
    jieba_result = pseg.cut(sentence)  # 标注句子分词后每个词的词性
   # print(sentence)
...
#    print(x[12],y[5])
    ...
    def _recon(i, j):
        ...
        if i == 0 or j == 0:
            ...
            return _recon(i - 1, j - 1) + [(x[i - 1], i)]#因为字母下标要少1，有0，所以这里减一
        elif table[i - 1, j] > table[i, j - 1]:
            return _recon(i - 1, j)
        else:
            return _recon(i, j - 1)

    recon_tuple = tuple(map(lambda x: x[0], _recon(i, j)))#根据提供的函数对指定序列做映射
    return recon_tuple
