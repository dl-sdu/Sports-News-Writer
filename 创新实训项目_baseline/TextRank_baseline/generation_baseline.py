...
try:
    ...
    pass
import codecs
from textrank import Rank_Sentence
...
#print(text1)
tr= Rank_Sentence()
tr.analyze(text=text1, lower=True, source = 'all_filters')
...
out_dict={}#最终输出数组字典
for item in tr.get_key_sentences(num=count//2):
    #print(item.index, item.weight, item.sentence)
    #fo.write(item.sentence)
   # print( item.sentence)
    if (re.findall(r'[0-9]+\.', item.sentence, flags=0)):
      m = re.findall(r'[0-9]+\.', item.sentence, flags=0)
      m = re.sub(r'\.', '', m[0], count=0, flags=0)
      out_dict[item.sentence] =int(m)
    else:
        out_dict[item.sentence] = 10000#设置完赛时间标号为10000
